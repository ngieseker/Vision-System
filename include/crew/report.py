...
class report(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(report._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != report._get_packed_fingerprint():
            raise ValueError("Decode error")
        return report._decode_one(buf)
    ...
    def _get_hash_recursive(parents):
        if report in parents: return 0
        tmphash = (0x4bcee7bb68e68f7e) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if report._packed_fingerprint is None:
            report._packed_fingerprint = struct.pack(">Q", report._get_hash_recursive([]))
        return report._packed_fingerprint
